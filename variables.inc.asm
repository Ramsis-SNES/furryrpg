...
; Labels without prefixes generally reference easily identifiable things
; (e.g. SNES registers, routine labels, jump tables, etc.)
...
; SNES register bits - INIDISP
.DEFINE kForcedBlank	%10000000
.DEFINE kINIDISP_00	$00						; brightness level 0 (screen black)
.DEFINE kINIDISP_01	$01						; etc.
.DEFINE kINIDISP_02	$02
.DEFINE kINIDISP_03	$03
.DEFINE kINIDISP_04	$04
.DEFINE kINIDISP_05	$05
.DEFINE kINIDISP_06	$06
.DEFINE kINIDISP_07	$07
.DEFINE kINIDISP_08	$08
.DEFINE kINIDISP_09	$09
.DEFINE kINIDISP_10	$0A
.DEFINE kINIDISP_11	$0B
.DEFINE kINIDISP_12	$0C
.DEFINE kINIDISP_13	$0D
.DEFINE kINIDISP_14	$0E
.DEFINE kINIDISP_15	$0F						; brightness level 15

; SNES register bits - BGMODE
.DEFINE kBGMODE_0	$00						; BG modes 0..7
.DEFINE kBGMODE_1	$01
.DEFINE kBGMODE_2	$02
.DEFINE kBGMODE_3	$03
.DEFINE kBGMODE_4	$04
...
.DEFINE kBGMODE_6	$06
.DEFINE kBGMODE_7	$07
.DEFINE kBG3priority	%00001000
.DEFINE kBG1TileSize	%00010000
.DEFINE kBG2TileSize	%00100000
.DEFINE kBG3TileSize	%01000000
.DEFINE kBG4TileSize	%10000000

; SNES register bits - TM/TS
.DEFINE kTM_BG1		%00000001
.DEFINE kTM_BG2		%00000010
.DEFINE kTM_BG3		%00000100
.DEFINE kTM_BG4		%00001000
.DEFINE kTM_OBJ		%00010000
.DEFINE kTS_BG1		%00000001
.DEFINE kTS_BG2		%00000010
.DEFINE kTS_BG3		%00000100
.DEFINE kTS_BG4		%00001000
.DEFINE kTS_OBJ		%00010000

; SNES register bits - SETINI
.DEFINE kVScan		%00000001					; 0 = no interlacing, 1 = interlacing
.DEFINE kObjVHigh	%00000010					; "IF "1" IS WRITTEN, THE OBJ SEEMS REDUCED HALF VERTICALLY IN APPEARANCE."
.DEFINE kBGV239		%00000100					; 0 = 224 lines, 1 = 239 lines
.DEFINE kPseudoH512	%00001000					; "SHIFT SUBSCREEN HALF DOT TO THE LEFT"
.DEFINE kEXTBG		%01000000
.DEFINE kExtSync	%10000000

; SNES register bits - NMITIMEN
.DEFINE kAutoJoy	%00000001
.DEFINE kHIRQ		%00010000
.DEFINE kHVIRQ		%00110000
.DEFINE kVIRQ		%00100000
.DEFINE kNMITIMEN_Enable	%10000000

; SNES register bits - MDMAEN/HDMAEN
.DEFINE kDMA_All	%11111111
.DEFINE kDMA_Channel0	%00000001
.DEFINE kDMA_Channel1	%00000010
.DEFINE kDMA_Channel2	%00000100
.DEFINE kDMA_Channel3	%00001000
.DEFINE kDMA_Channel4	%00010000
.DEFINE kDMA_Channel5	%00100000
.DEFINE kDMA_Channel6	%01000000
.DEFINE kDMA_Channel7	%10000000

; SNES register bits - WRIO
.DEFINE kWRIO_Joy1	%01000000
.DEFINE kWRIO_Joy2	%10000000

; SNES register bits - MEMSEL
.DEFINE kFastROM	%00000001
...
.ENUM 0									; argument(s) (8/16 bit) / effect(s) (if not self-explanatory)
	ZERO_END			DB				; none / end event script processing
	BG3_TEXT			DB				; y position (8), x position (8), string pointer (24) / prints a string on BG3
	DIALOGUE			DB				; dialogue pointer (16)
	DISABLE_HDMA_CH			DB				; HDMA channel(s) no. # (8)
	DMA_ROM2CGRAM			DB				; CGRAM target address (8), ROM source address (16), ROM source bank (8), size (16)
	DMA_ROM2VRAM			DB				; VRAM target address (16), ROM source address (16), ROM source bank (8), size (16)
	DMA_ROM2WRAM			DB				; WRAM target address (16), WRAM target bank (8), ROM source address (16), ROM source bank (8), size (16)
	ENABLE_HDMA_CH			DB				; HDMA channel(s) no. # (8)
	GSS_COMMAND			DB				; command (16), parameter (16)
	GSS_LOAD_TRACK			DB				; track no. # (16)
	INIT_GAMEINTRO			DB				; none
	JSUBL				DB				; address (24) / go to some subroutine (long)
	JSUBR				DB				; address (16) / go to some subroutine
	JUMP				DB				; position in event script to jump to (16)
	LOAD_AREA			DB				; no. # of area (16)
	LOAD_PARTY_FORMATION		DB				; no. # of party formation
	MONITOR_INPUT_JOY1		DB				; joypad data (16), position in event script to jump to (16)
;	MONITOR_INPUT_JOY2		DB				; joypad data (16), position in event script to jump to (16)
	MOVE_ALLY			DB				; ally no. #, direction(s), speed
	MOVE_HERO			DB				; target screen position (16), speed (8) // caveat: position difference has to be divisible by speed value
	MOVE_NPC			DB				; NPC no. #, direction(s), speed
	MOVE_OBJ			DB				; obj. no. #, direction(s), speed
	MSU_LOAD_TRACK			DB				; track no. # (16)
	MSU_TRACK_FADEIN		DB				; speed
	MSU_TRACK_FADEOUT		DB				; speed
	MSU_TRACK_PLAY			DB				; 000000rpvvvvvvvv [r = Repeat flag, p = Play flag, v = volume] (16)
	MSU_TRACK_STOP			DB				; none
	SCR_EFFECT			DB				; effect no. #
	SCR_EFFECT_TRANSITION		DB				; transition effect no. #, speed
	SCR_SCROLL			DB				; BG(s), direction(s), speed
	SET_REGISTER			DB				; SNES register (16), value (8)
	SIMULATE_INPUT_JOY1		DB				; joypad data (16)
	SIMULATE_INPUT_JOY2		DB				; joypad data (16)
	TOGGLE_AUTO_MODE		DB				; none
	WAIT_JOY1			DB				; joypad data (16)
	WAIT_JOY2			DB				; joypad data (16)
	WAIT_FRAMES			DB				; no. of frames (16)
	WRITE_RAM_BYTE			DB				; address (24), value (8) // write a byte to WRAM (for setting register mirror variables etc.)
.ENDE
...
.DEFINE kTextBG3		$0000					; used as index into FillTextBuffer, order (ascending value) has to match
...
.DEFINE kTextBoxLine1		$02E7					; tilemap entry for start of text box line 1
.DEFINE kTextBoxLine2		$0307					; tilemap entry for start of text box line 2
.DEFINE kTextBoxLine3		$0327					; tilemap entry for start of text box line 3
.DEFINE kTextBoxLine4		$0347					; tilemap entry for start of text box line 4
.DEFINE kTextBoxLineLength	46					; no. of characters/half-tiles per text box line
.DEFINE kTextBoxSelLine1	%00000001				; selection on text box line 1
.DEFINE kTextBoxSelLine2	%00000010				; selection on text box line 2
.DEFINE kTextBoxSelLine3	%00000100				; selection on text box line 3
.DEFINE kTextBoxSelLine4	%00001000				; selection on text box line 4
.DEFINE kTextBoxTileLine1	0					; tile no. at start of text box line 1
.DEFINE kTextBoxTileLine2	23					; tile no. at start of text box line 2
.DEFINE kTextBoxTileLine3	46					; tile no. at start of text box line 3
.DEFINE kTextBoxTileLine4	69					; tile no. at start of text box line 4
...
; Language constants in the form "kLangXXX" where XXX is an ISO 639-3 language code.
; The order of dialogue banks must be the same as the order of language constants below.
; Self-reminder: The dialogue routine acknowledges game language differently when loading pointers
; than do name loading routines like in area or main menu code, maybe FIXME
.ENUMID 0
.ENUMID kLangENG
;.ENUMID kLangENG2							; might need to add these if/when multiple banks per language are used
;.ENUMID kLangENG3
.ENUMID kLangDEU
;.ENUMID kLangDEU2
;.ENUMID kLangDEU3
...
; Effect types
.ENUMID 0 STEP 2							; order has to match SRC_EffectTypes table
...
.ENUMID $FF STEP -1							; negative values/decrementing .ENUMID
...
.DEFINE kGameConfigU16	%00000100
.DEFINE kGameConfigVRAM	%00001000

.DEFINE kGameStateGoodROM	%00000001
;.DEFINE kGameState1	%00000010
;.DEFINE kGameState2	%00000100
;.DEFINE kGameState3	%00001000
...
; $0000-$01FF: Reserved for stack (for now)
...
; Self-reminder: If/when the Direct Page register is set to something other than $0000, (at least)
; 16-bit operand hints must be used when addressing P00/struct_RAM_0000_00FF, or else WLA DX will
; default to 8-bit addressing here (Direct Page address space, cf. UnusedCode@Accumulator)!
...
.STRUCT struct_RAM_0100_01FF SIZE 256
	Reserved			dsb 255
	InitStack			db				; initial stack pointer = $01FF
.ENDST
...
;	BRR_stream_list			dsb 4				; SNESGSS
	...
	GameTimeSecond			db				; 1 game time second = 1 frame (??)
	GameTimeMinute			db
	GameTimeHour			db
	GameTimeDay			db				; or rather dw?
	...
	Mode7_Altitude			db				; altitude setting, currently 112 values ($00-$6F)
	Mode7_BG2HScroll		db
	...
	RingMenuAngle			db				; holds current ring menu rotation angle (used as anchor point for many elements)
	RingMenuRadius			db				; holds current ring menu radius in pixels (varies during animations)
...
;	SPC_DataOffset			dw				; 16-bit address of source data to be uploaded to SPC700
	...
	VWF_BitsUsed			dw				; holds no. of bits used up in current VWF tile, also used for calculating bit shifting
	VWF_BufferIndex			dw				; holds current VWF buffer index position
	VWF_Loop			db				; loop counter for VWF routine
...
.STRUCT struct_RAM_0A00_1FFF SIZE $1600					; the SIZE here might get relevant later on (e.g., should we decide to convert the WRAM .ENUMs into .RAMSECTIONs)
	...
	JumpIRQ				dw				; holds address of current IRQ subroutine
	JumpNMI				dw				; holds address of current NMI/Vblank subroutine
	...
	Time_Year			db
	Time_Century			db
	TimeoutCounter			dw				; used by SNESGSS routines for SPC700 communication timeout
.ENDST
...
; .ENUM omits that. However, we *do* need the respective high byte in specific cases (e.g. for the
; "set" macro or for conveniently setting data source addresses within a DP), so we need to address
; all DPx variables with an 8-bit operand hint, like "lda <DP2.Temp" or "lda DP2.Temp.#b".
...
; WRAM area $7E2000-$7FFFFF is generally addressed with the corresponding bank byte, so the
; following .ENUM includes that.

.ENUM $7E2000
	RAM				INSTANCEOF struct_RAM_AllUpper
.ENDE

; The following .DEFINEs help keep struct_RAM_Routines more clean, and allow for shorter variable
; names.
