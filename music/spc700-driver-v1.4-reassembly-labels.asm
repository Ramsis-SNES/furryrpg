...
;
; Disassembly and adjustments for reassembly with WLA-SPC700 by Ramsis, July 2022.
...
;
; To assemble:
; wla-spc700 -v -o spc700-driver-v1.4-reassembly-labels.o spc700-driver-v1.4-reassembly-labels.asm
; wlalink -b -v spc700-driver-v1.4-reassembly-labels.lnk spc700-driver-v1.4-reassembly-labels.bin
...
sampleADSRPtr:
	.DW	$0b12				; 16-bit pointer
;	.DW	sampleADSRAddr			; //$0208

soundEffectsDataPtr:
	.DW	$0b24				; 16-bit pointer
;	.DW	0				; //$020a

musicDataPtr:
	.DW	$0b25				; 16-bit pointer
;	.DW	musicDataAddr			; //$020c

editorInit:
@_020e:	call	!initDSPAndVars

	mov	x, #1
	mov	D_STEREO, x			; //enable stereo in the editor, mono by default
	dec	x
	call	!startMusic
...
@_06bf:	mov	D_STEREO, x			; //mono by default
...
.REPEAT 35					; 35 zeroes align the start of the following table to address $0B00 (in sound RAM) = $0902 (in driver .BIN file)
	.DB 0
.ENDR
